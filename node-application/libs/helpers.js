...
const getDnsRecords = async () => {
    try {
        console.log('process.env.CLOUDFLARE_KEY', process.env.CLOUDFLARE_KEY);
        ...
        if (data.success) {
            ...
            errorLog.error(`[ERROR] - [GET][getDnsRecords] Not found correct searched name in your zones.`);
        } else {
...
const updateCloudFlareDnsData = async (currentIp, dnsRecord) => {
    try {
        ...
        if (currentIp === fileData.lastIpAddress) {
            ...
            const response = await fetch(`https://api.cloudflare.com/client/v4/zones/${dnsRecord.zone_id}/dns_records/${dnsRecord.id}`, {
                method: 'put',
                headers: {
                    'Content-Type': 'application/json',
                    'X-Auth-Email': process.env.CLOUDFLARE_USERNAME,
            ...
            if (data.success) {
                ...
                try {
                    await helperFiles.writeFile('./data/store.json', JSON.stringify(fileData));
